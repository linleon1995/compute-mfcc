...
//void process_wave_to_bytes(std::ifstream& wavFp, char* buf) {
//    // Read the wav header    
//    wavHeader hdr;
...
//    }
//
//    int64_t bigbufferLength = inputData.size();
//    int16_t* bigbuffer = new int16_t[bigbufferLength];
//    for (int i = 0; i < bigbufferLength; i++) {
//        bigbuffer[i] = inputData[i];
//    }
//
//
//    vector a;
...
std::vector<int16_t> get_data(const char* wavPath) {
    ...
    //wavFp.seekg(0, std::ios::end);
    //size_t length = wavFp.tellg();
    //const int length_int = static_cast<int>(length);
    //wavFp.seekg(0, std::ios::beg);

    //std::vector<char> vTBuffer(std::istreambuf_iterator<char>(wavFp), 
...
//std::vector<float> loadModelFile::run(char* buf, int32_t buf_len) {
//    // Initialise buffer
//    uint16_t bufferLength = 128;
//    int16_t* buffer = new int16_t[bufferLength];
//    int bufferBPS = (sizeof buffer[0]);
//
//    int x = bufferLength * bufferBPS;
//
//    /*std::vector<int16_t> inputData;
//    while (wavFp.gcount() == bufferLength * bufferBPS && !wavFp.eof()) {
//        for (int i = 0; i < bufferLength; i++)
...
//    for (int i=0; i<buf_len; i+=x) {
//        waveform.push_back(buf[i]);
//    }
//
//    // preprocess
