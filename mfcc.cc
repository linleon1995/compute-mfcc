...
////
////
////std::vector<float> process_wave_test(const char* filePath)
////{
////    wavHeader wav_hdr;
...
////    {
////        //Read the data
////        uint16_t bytesPerSample = wav_hdr.bitsPerSample / 8;      //Number     of bytes per sample
////        uint64_t numSamples = wav_hdr.ChunkSize / bytesPerSample; //How many samples are in the wav file?
...
////            inputData.insert(inputData.end(), temp.begin(), temp.end());
...
////
////
////std::vector<float> process_wave(std::ifstream& wavFp) {
////    // Read the wav header    
////    wavHeader hdr;
...
////        //std::vector<float> temp = processFrame_vect(buffer, bufferLength);
////        //std::vector<float> temp = prevsamples;
////        std::vector<float> temp;
////        for (int i = 0; i < bufferLength; i++)
////            temp.push_back(buffer[i]);
////        inputData.insert(inputData.end(), temp.begin(), temp.end());
...
//
//    // Read input file stream, extract MFCCs and write to output file stream
//    v_d process_and_return(std::ifstream& wavFp) {
//        // Read the wav header    
//        wavHeader hdr;
//        int headerSize = sizeof(wavHeader);
//        wavFp.read((char*)&hdr, headerSize);
